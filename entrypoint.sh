...
MM_PASSWORD=${MM_PASSWORD:-mattermost}
...
if [ "$1" = 'mattermost' ]; then
  ...
  then
    ...
    cp /mattermost/config-backup/config.json $MM_CONFIG
  ...
  then
    ...
    export MM_SQLSETTINGS_DATASOURCE="${MM_SQLSETTINGS_DRIVERNAME}://$MM_USERNAME:$ENCODED_PASSWORD@$DB_HOST:$DB_PORT_NUMBER/$MM_DBNAME?sslmode=disable&connect_timeout=10"
    echo OK
  else
